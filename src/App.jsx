  ...
  return (
    <div class="min-h-screen bg-gradient-to-br from-gray-800 to-gray-900 p-4">
      <div class="max-w-3xl mx-auto">
        <h1 class="text-4xl font-bold text-white mb-8 text-center">New Born Name Finder</h1>
        <form onSubmit={handleGenerateNames} class="bg-gray-800 p-6 rounded-lg shadow-md space-y-4">
          <div>
            <label class="block text-gray-200 font-semibold mb-2">Gender</label>
            <select
              class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent box-border cursor-pointer"
              value={gender()}
          ...
          <div>
            <label class="block text-gray-200 font-semibold mb-2">Origin</label>
            <input
              type="text"
              placeholder="e.g., Hebrew, Latin, etc."
              class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent box-border"
              value={origin()}
              onInput={(e) => setOrigin(e.target.value)}
            />
          </div>
          <div>
            <label class="block text-gray-200 font-semibold mb-2">Meaning</label>
            <input
              type="text"
        ...
        <Show when={names().length > 0}>
          <div class="mt-8 bg-gray-800 p-6 rounded-lg shadow-md">
            <h2 class="text-2xl font-bold text-white mb-4">Name Suggestions</h2>
            <ul class="list-disc pl-6 space-y-2">
              <For each={names()}>
                {(name) => (
                  <li class="text-gray-200">{name}</li>
                )}
              </For>
            </ul>
          </div>
